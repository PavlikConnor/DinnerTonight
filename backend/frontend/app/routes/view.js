...
export default Ember.Route.extend({
  ...
  async model(params) {
    let recipe = (await this.get('ajax').request('/recipe/'+params.pk))[0];
    //console.log(recipe);
    let ingredients = await this.get('ajax').request('/ingredients/'+params.pk);
    //console.log(ingredients);
    let recIngredients = await this.get('ajax').request('/recIngredients/'+params.pk);
