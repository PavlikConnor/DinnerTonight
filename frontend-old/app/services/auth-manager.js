...
export default Ember.Service.extend({
	...
	login: function(){
		...
		//make api request
		Ember.$.post('http://localhost/api/session', data, function(response){
...
//console.log(auth.isLoggedIn);
	...
	logout: function(){
		...
		var auth = this;
		Ember.$.ajax({url: 'http://localhost/api/session', type: 'DELETE'}).then(
			function(response){
	...
	**/
	init: function(){
		...
		Ember.$.get('http://localhost/api/session', function(response){
			if(response.isauthenticated){
				//success
